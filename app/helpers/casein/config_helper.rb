module Casein
  module ConfigHelper
    ...
    # The initial page the user is shown after they sign in or click the logo. Probably
    # this should be set to the first tab.
    # Do not point this at casein/index!
    def casein_config_dashboard_url
      url_for controller: :casein, action: :blank
    end

    # A list of stylesheets to include. Do not remove the core casein/casein,
    # but you can change the load order, if required.
    def casein_config_stylesheet_includes
      %w(casein/casein casein/custom)
    end

    # A list of JavaScript files to include. Do not remove the core casein/casein, but you
    #  can change the load order, if required.
